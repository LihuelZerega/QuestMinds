...
function GeneralKnowledgeEasy() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      const response = await axios.get<APIResponse>(
        `https://opentdb.com/api.php?amount=5&category=9&difficulty=${difficulties[difficultyIndex]}&type=boolean`
      );
  ...
  const goToNextLevel = () => {
    router.push("/ruta-para-siguiente-nivel");
  };

  if (currentIndex >= questions.length) {
    return (
      <motion.div
        className="flex flex-col items-center justify-center min-h-screen blur-effect mx-6 sm:mx-0"
        initial="hidden"
        animate="visible"
        exit="exit"
        variants={fadeIn}
    ...
    >
      <div className="text-center container mx-auto max-w-xl">
        <div className="flex flex-row items-center justify-center font-bold text-xl mb-24">
          {/* CATEGORIA */}
          <h1 className="text-white text-2xl">Art: {""}</h1>
          {/* DIFFICULTY */}
...
export default GeneralKnowledgeEasy;
