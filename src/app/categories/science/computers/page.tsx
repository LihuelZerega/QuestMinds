...
function GeneralKnowledgeEasy() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      const response = await axios.get<APIResponse>(
        `https://opentdb.com/api.php?amount=5&category=18&difficulty=${difficulties[difficultyIndex]}&type=boolean`
      );
  ...
  const goToNextLevel = () => {
    router.push("https://questminds.vercel.app/categories/science/computers/medium");
  };
    ...
    >
      <div className="text-center container mx-auto max-w-xl">
        <div className="flex flex-row items-center justify-center font-bold text-xl mb-24">
          {/* CATEGORIA */}
          <h1 className="text-white text-2xl">Computers: {""}</h1>
          {/* DIFFICULTY */}
